 ...
 * preference about how you like to work, and it travels with the workspace to a
 * new machine along with everything else.
 *
 * There is only one theme now. This stays because a stored id still has to be
 * read and honoured — a workspace last opened by an older build has a name like
 * `midnight` in it, and `themeById` falls back to the first theme rather than
...
/** Reads the stored preference, applies it, and returns a setter that persists. */
